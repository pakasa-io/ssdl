...
[![Spec version](https://img.shields.io/badge/spec-v1.5.0-blue)](CHANGELOG.md)
...
| [`scripts/bundle.py`](scripts/bundle.py) | The bundler — assembles `spec/` into `ssdl.spec.md` and `agents/agent.manifest.yml`; `python3 scripts/bundle.py --check` verifies the round-trip. Runs from any directory. |
...
| [`agents/agent.manifest.yml`](agents/agent.manifest.yml) | The agent index — *where to find what*, for progressive loading. **Generated.** |
| [`agents/AGENT_PROTOCOL.md`](agents/AGENT_PROTOCOL.md) | The contract for AI agents and tooling that consume the split spec. |
...
The spec is versioned independently of any screen. The current language version is **1.5.0** — see [CHANGELOG.md](CHANGELOG.md) for the full per-version history. Individual screens and fragments declare their own version (`SCREEN Login v1`, `FRAGMENT navigation v2`), recorded in each file's `META.changelog` / `FRAGMENT_META.changelog`.
