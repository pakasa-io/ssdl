...
Every path is relative to the **root of the spec tree** — the directory that holds `spec/` (the repository root, or,
for a vendored copy, the folder that contains `agent.manifest.yml`). It is **not** relative to this document's own
location — so pointers resolve identically wherever the index is read from.
