...
- **Template vs runtime.** The skill ships an empty scaffold + schema (`skills/to-ssdl/kb/`, generated by
  `_build.py`); each project gets its **filled** copy here under `app-spec/kb/`, instantiated on first run.
- **Fill, then consume.** **Phase 2** extracts facts into each card (`facts` + `extracted_from`); **Phase 5** authors
  each section from its card's facts + spec slice, appending the consuming screen to each fact's `applies_to`
  (populated **retrospectively**). A card's `feeds` routes its facts to the downstream sections that consume them.
